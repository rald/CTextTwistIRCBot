...
const char *inet_ntop(int af, const void *src, char *dst, socklen_t size)
{
	...
	res =
	    WSAAddressToStringA(&addr.sa, sizeof(addr), 0, dst,
				(LPDWORD) & size);
	if (res != 0)
		return NULL;
	return dst;
...
static void select_grow(SelectSet * s)
{
	...
	for (i = 0; i < SELECT_MAX; i++) {
		s->fds[i] =
		    dyad_realloc(s->fds[i], s->capacity * sizeof(fd_set));
...
static void updateStreamTimeouts(void)
{
	...
	while (stream) {
		if (stream->timeout) {
			if (currentTime - stream->lastActivity >
			    stream->timeout) {
				stream_emitEvent(stream, &e);
				dyad_close(stream);
			}
...
static void stream_initAddress(dyad_Stream * stream)
{
	...
	if (addr.sas.ss_family == AF_INET6) {
		...
		inet_ntop(AF_INET, &addr.sai.sin_addr, stream->address,
			  INET_ADDRSTRLEN);
...
void dyad_update(void)
{
	...
	while (stream) {
		...
		case DYAD_STATE_CONNECTED:
			select_add(&dyad_selectSet, SELECT_READ,
				   stream->sockfd);
			if (!(stream->flags & DYAD_FLAG_READY)
			    || stream->writeBuffer.length != 0) {
				select_add(&dyad_selectSet, SELECT_WRITE,
					   stream->sockfd);
			}
			break;
		...
		case DYAD_STATE_CONNECTING:
			select_add(&dyad_selectSet, SELECT_WRITE,
				   stream->sockfd);
			select_add(&dyad_selectSet, SELECT_EXCEPT,
				   stream->sockfd);
			break;
...
#pragma warning(pop)
	...
	select(dyad_selectSet.maxfd + 1,
	       ...
	       dyad_selectSet.fds[SELECT_EXCEPT], &tv);
	...
	while (stream) {
		...
		case DYAD_STATE_CONNECTED:
			if (select_has
			    (&dyad_selectSet, SELECT_READ, stream->sockfd)) {
		...
		case DYAD_STATE_CLOSING:
			if (select_has
			    (&dyad_selectSet, SELECT_WRITE, stream->sockfd)) {
		...
		case DYAD_STATE_CONNECTING:
			if (select_has
				...
				dyad_Event e;
				getsockopt(stream->sockfd, SOL_SOCKET, SO_ERROR,
					   &optval, &optlen);
				if (optval != 0)
					goto connectFailed;
			    ...
			    if (select_has
				(&dyad_selectSet, SELECT_EXCEPT, stream->sockfd)
			    ) {
...
double dyad_getTime(void)
{
	...
	return (ft.dwHighDateTime * 4294967296.0 / 1e7) +
